...
## 5. ASO（App Store Optimization）設定

> ストア掲載情報はすべて `fastlane/metadata/` に管理されています。

### 5-0. ASO 戦略サマリー

| 項目 | iOS (App Store Connect) | Android (Google Play Console) |
|---|---|---|
| **アプリ名** | キラリ - 体重管理 & AI日記 | キラリ - 体重管理 & AI日記 |
| **サブタイトル** | かわいく続くダイエット記録アプリ | ―（なし） |
| **短い説明** | ―（なし） | AIが毎日やさしく励ます体重管理アプリ… |
| **カテゴリ** | ヘルスケア＆フィットネス | 健康＆フィットネス |
| **キーワード** | 体重管理,ダイエット,体重記録… | 説明文に自然に含める |
| **メタデータ場所** | `fastlane/metadata/ios/ja/` | `fastlane/metadata/android/ja-JP/` |
...
|---|---|---|
| 体重管理 | 高 | タイトルに含める |
| ダイエット | 高 | サブタイトル・説明文に含める |
| 体重記録 | 中 | キーワード・説明文に含める |
| 体重日記 | 中 | キーワードに含める |
| AI日記 | 低 | タイトルに含める（差別化） |
| レコーディングダイエット | 中 | キーワード・説明文に含める |
| 体重グラフ | 低 | キーワード・説明文に含める |
| かわいい 体重アプリ | 低 | サブタイトル・説明文に含める |

**iOS キーワード（100文字以内・カンマ区切り）:**
```
体重管理,ダイエット,体重記録,体重日記,AI,健康管理,体重ログ,レコーディング,体重グラフ,かわいい,ダイエットアプリ
```

### 5-0-2. スクリーンショット ASO 最適化

スクリーンショットのキャプション（上部テキスト）に以下のキーワードを含めること:

| 枚目 | キャプション例 | 画面 |
|---|---|---|
| 1枚目 | **AIが毎日やさしく励ます** | ホーム画面（AIコメント表示） |
| 2枚目 | **ワンタップでかんたん体重記録** | 記録画面 |
| 3枚目 | **体重グラフで変化が一目瞭然** | ホーム画面（グラフ表示） |
| 4枚目 | **AI日記であなたの歩みを振り返り** | 日記画面 |
| 5枚目 | **続けるほどペットが成長！** | ペット画面 |

### 5-0-3. ASO 改善チェックリスト

- [ ] タイトルにメインキーワード「体重管理」が含まれている
- [ ] サブタイトル（iOS）にサブキーワード「ダイエット」が含まれている
- [ ] 短い説明（Android）が80文字以内で訴求力がある
- [ ] 説明文の冒頭100文字にキーワードが自然に含まれている
- [ ] スクリーンショットにキャプションテキストが載っている
- [ ] スクリーンショットが最低5枚用意されている
- [ ] 1枚目のスクリーンショットが最もインパクトのある画面
- [ ] 競合アプリ（あすけん、SmartDiet、RecStyle）との差別化ポイントが説明文に含まれている

---

## 6. App Store（iOS）リリース 残り設定手順
...
- [ ] アプリ名: `キラリ - 体重管理 & AI日記`（← ASO最適化済み）
- [ ] サブタイトル: `かわいく続くダイエット記録アプリ`（← ASO最適化済み）
- [ ] キーワード: `fastlane/metadata/ios/ja/keywords.txt` を参照
- [ ] 説明文: `fastlane/metadata/ios/ja/description.txt` を参照
- [ ] プロモーションテキスト: `fastlane/metadata/ios/ja/promotional_text.txt` を参照
...
### 6-3. ATT（App Tracking Transparency）
...
## 7. Google Play（Android）リリース 残り設定手順
...
- [ ] アプリ名: `キラリ - 体重管理 & AI日記`（← ASO最適化済み）
- [ ] 短い説明: `fastlane/metadata/android/ja-JP/short_description.txt` を参照
- [ ] 詳しい説明: `fastlane/metadata/android/ja-JP/full_description.txt` を参照
- [ ] カテゴリ: 健康＆フィットネス
